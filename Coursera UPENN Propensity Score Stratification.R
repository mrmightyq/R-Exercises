...
#read in data
library(readr)
rhc <- read_csv("C:/Users/KnudseQ/Desktop/R Data/rhc.csv")
#view data
View(rhc)
...
# fit propensity score model to get weights, but truncated at 99 and 1 percentile
weightmodel<-ipwpoint(exposure= treatment, family = "binomial", link ="logit",
                      denominator= ~ age + female + meanbp1+ARF+CHF+Cirr+colcan+
...
model2 <-glm(lwage ~ IQ,data=card.data)
summary(model2)

ivmodel=ivreg(lwage ~ IQ+ educ12 + exper + reg661 + reg662 +
                reg663 + reg664 + reg665+ reg666 + reg667 + reg668, 
              ~ nearc4 + educ12 + exper +
                reg661+ reg662 + reg663 + reg664 + reg665 + reg666 +
                reg667 + reg668, x=TRUE, data=card.data)
